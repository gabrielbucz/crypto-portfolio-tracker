...
async function carregarMoedasSelect() {
    ...
    `).join('');
        ...
        select.addEventListener('change', () => {
            const moeda = moedasMercado.find(m => m.id === select.value);
            if (moeda) {
                document.getElementById('input-preco').placeholder =
...
function calcularResumo(transacoes) {
    ...
    transacoes.forEach(t => {
        ...
        grupos[t.coinId].totalQtd += Number(t.quantidade);
        grupos[t.coinId].totalGasto += Number(t.quantidade) * Number(t.preco);
    });
    ...
    const linhas = Object.values(grupos).map(g => {
        ...
        const precoMedio = g.totalGasto / g.totalQtd;
