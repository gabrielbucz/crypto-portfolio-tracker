## 🚀 Tecnologias Utilizadas

### 🧩 Framework
- Bootstrap 5.3.2

### 🌐 API
-CoinGecko API

---

## 🎯 Justificativa das escolhas

### Framework (Bootstrap 5.3.2)
O Bootstrap foi escolhido por permitir o desenvolvimento rápido de interfaces responsivas e modernas, com um sistema de grid eficiente e componentes prontos. Isso reduz o tempo de desenvolvimento e garante um padrão visual consistente.

### API  (CoinGecko)
A API foi escolhida por ser gratuita, confiável e fornecer dados atualizados em tempo real. Isso permite criar uma aplicação com valor real para o usuário, especialmente em cenários como monitoramento de dados (ex: criptomoedas e moedas).

---

## 💡 Objetivo do Projeto
Monitorar o mercado crypto e também um portifólio com compras, vendas, lucro e prejuízo.


## 📋 Checklist de Indicadores de Desempenho

### 🎨 RA1 - Layout Responsivo e CSS

* [x] ID 01 - Protótipo adaptável (mobile e desktop)
* [x] ID 02 - Layout com Framework CSS
* [x] ID 03 - Layout com CSS puro (Flexbox/Grid)
* [x] ID 04 - Uso de componentes do framework
* [x] ID 05 - Uso de unidades relativas
* [x] ID 06 - Design System consistente
* [x] ID 07 - Uso de Sass (SCSS)
* [x] ID 08 - Tipografia responsiva
* [x] ID 09 - Responsividade de imagens
* [x] ID 10 - Otimização de imagens

---

### 📝 RA2 - Formulários e Validação

* [x] ID 11 - Validação HTML nativa
* [x] ID 12 - Validação com REGEX
* [x] ID 13 - Uso de inputs (checkbox, radio, select)
* [x] ID 14 - Uso de Web Storage

---

### ⚙️ RA3 - Processo de Desenvolvimento

* [x] ID 15 - Ambiente com Node/NPM
* [x] ID 16 - Versionamento com Git/GitHub
* [x] ID 17 - README padronizado
* [x] ID 18 - Organização modular de arquivos
* [x] ID 19 - Uso de ESLint/Prettier

---

### 🧩 RA4 - Interatividade com JavaScript

* [x] ID 20 - Uso de jQuery
* [x] ID 21 - Plugin jQuery

---

### 🔌 RA5 - Consumo de APIs

* [x] ID 22 - Requisição para API fake (POST/CRUD)
* [x] ID 23 - Exibição de dados da API fake
* [x] ID 24 - Consumo de API pública

